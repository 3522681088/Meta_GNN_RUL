...
SCRIPT_VERSION = "experimentA25_0b_execution_provenance_recovery_v1"
...
def meta_summary(aggregate: Mapping[tuple[str, str], list[float]], json_files: list[Path]) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    for method in METHODS:
        for field in REQUIRED_META_FIELDS:
            ...
            rows.append({"method": method, "quantity": field, "value": value, "unit": unit,
                         "availability": "available", "evidence": f"Recovered from {len(json_files)} JSON artifacts; raw range min={minimum:g}, max={maximum:g}.",
...
def main() -> int:
    ...
    for source in (a241_evidence, a242_evidence):
        for key, values in source.items():
            raw_evidence[key].extend(values)
    provenance = pd.concat([a241_inventory, a242_inventory], ignore_index=True)
    checkpoints = checkpoint_inventory(checkpoint_files, selected, args.checkpoint_validation)

    summary_rows = pft_summary(a23_training) + meta_summary(raw_evidence, a241_json + a242_json)
    ...
    complete = not provenance.empty and bool(all(payload.get("complete") is True for payload in decisions.values()))
