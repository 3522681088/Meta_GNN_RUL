...
SCRIPT_VERSION = "experimentA25_0c_stratified_checkpoint_parameter_audit_v2"
METHODS = ("pretrain_finetune_k", "meta_no_graph_k", "meta_gnn_k")
CHECKPOINT_SUFFIXES = {".pt", ".pth", ".ckpt"}
MANIFEST_FILE_SUFFIXES = {
    ".csv", ".json", ".txt", ".pt", ".pth", ".ckpt", ".yaml", ".yml", ".npz", ".npy"
}
...
def validate_manifest(root: Path, experiment: str) -> tuple[Path, dict[str, Any], list[dict[str, str]]]:
    ...
    for name, expected in payload.get("artifacts", {}).items():
        # Some historical manifests mixed immutable metadata into `artifacts`,
        # e.g. {"config_sha256": "<hash>", "protocol_dir": "<path>"}.
        # Such entries are not artifact filenames and must not be resolved
        # relative to the output directory.
        suffix = Path(str(name)).suffix.lower()
        expected_text = str(expected)
        is_file_hash_entry = (
            suffix in MANIFEST_FILE_SUFFIXES
            and re.fullmatch(r"[0-9a-fA-F]{64}", expected_text) is not None
        )
        if not is_file_hash_entry:
            checks.append({
    ...
    failures = [row for row in checks if row["status"] in {"missing", "mismatch"}]
    if failures:
...
def main() -> int:
    ...
    artifact_audit_complete = bool(
        ...
        and all(row["status"] in {"passed", "metadata_not_file"} for row in integrity_rows)
    ...
    integrity = {
        ...
        "all_manifest_artifact_checks_passed": all(
            row["status"] in {"passed", "metadata_not_file"} for row in integrity_rows
        ),
        "upstream_complete": {key: value.get("complete") is True for key, value in upstream_decisions.items()},
