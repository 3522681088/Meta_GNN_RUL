...
def run_worker(args: argparse.Namespace) -> None:
    ...
    # ``pretrained_model`` and ``source_model`` refer to the same object after
    # train_source_steps().  Both references must be released before the
    # target branches are constructed; otherwise the source learner remains on
    # the GPU throughout every target evaluation and needlessly holds memory.
    del pretrained_model, source_model
    if torch.cuda.is_available():
    ...
    for shot in args.shots:
        ...
        for regime_index, regime in enumerate(args.regimes):
            ...
            if regime != "source_only":
                ...
                # The other branches move the model in train_epochs().  The
                # source-only branch has no target optimisation, so it must do
                # this explicitly before evaluate() transfers its input to the
