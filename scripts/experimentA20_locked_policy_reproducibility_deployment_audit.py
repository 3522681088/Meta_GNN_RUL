...
SCRIPT_VERSION = "experimentA20_locked_policy_reproducibility_deployment_audit_v2"
QUESTION = (
...
def choose_metric_columns(frame: pd.DataFrame, metric: str, path: Path) -> tuple[str, str]:
    ...
    # Pair tables also contain helper columns such as
    # ``nasa_score_delta_candidate_minus_baseline``.  They mention the word
    # baseline but are not baseline metric values, so exclude every derived
    # delta/relative/win column symmetrically for both arms.
    derived_tokens = ("delta", "relative", "win")
    baselines = [
        name
        for name in metric_names
        if "baseline" in name.lower()
        and not any(token in name.lower() for token in derived_tokens)
    ]
    blends = [
        name
        for name in metric_names
