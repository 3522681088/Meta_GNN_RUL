...
SCRIPT_VERSION = "experimentA25_1b_same_architecture_compute_accounted_selection_pilot_v3"
...
def load_contract(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, pd.DataFrame], dict[str, str]]:
    ...
    required_protocol_fields = {
        "registered_primary_question", "model_seeds", "support_split_seeds", "shots",
        "expected_source_episode_records", "expected_A25_1b_worker_cells",
        "expected_A25_1b_training_records", "source_gradient_updates_per_method_cell",
        "source_window_presentations_per_method_cell", "target_epochs", "inner_steps",
        "outer_steps", "inner_learning_rate", "outer_learning_rate", "config_contract",
        "training_file_inventory",
    }
    if missing := required_protocol_fields - set(protocol):
        raise A251bError(f"A25.1a protocol lacks required fields: {sorted(missing)}")
    # Validate the implicit A25.1a episode dimension before any model or data work.
    episodes_per_source_domain_phase(protocol)
...
def load_config(protocol: Mapping[str, Any], config_arg: Path) -> tuple[dict[str, Any], Path]:
    ...
    cfg["inner_steps"] = int(protocol["inner_steps"])
    cfg["outer_steps"] = int(protocol["outer_steps"])
    cfg["target_epochs"] = int(protocol["target_epochs"])
    cfg["pair_aux_weight"] = float(cfg.get("pair_aux_weight", 0.0))
    if (
        cfg["batch_size"] < 1
        or cfg["window_size"] < 2
        or cfg["rul_cap"] <= 0
        or cfg["inner_steps"] < 1
        or cfg["outer_steps"] < 1
        or cfg["target_epochs"] < 1
        or cfg["inner_lr"] <= 0
        or cfg["outer_lr"] <= 0
    ):
...
def episodes_per_source_domain_phase(protocol: Mapping[str, Any]) -> int:
    """Recover the locked per-cell episode count from auditable A25.1a totals.

    A25.1a v1 stores the total source-episode record count but deliberately does
    not duplicate its CLI-only ``episodes_per_source_domain`` setting in the
    protocol.  Deriving it from the factorial dimensions prevents an unstated
    default from entering A25.1b.
    """
    required = {
        "expected_source_episode_records",
        "expected_A25_1b_worker_cells",
        "model_seeds",
        "support_split_seeds",
    }
    if missing := required - set(protocol):
        raise A251bError(
            f"A25.1a protocol lacks episode-accounting fields: {sorted(missing)}"
        )

    model_seeds = tuple(int(value) for value in protocol["model_seeds"])
    split_seeds = tuple(int(value) for value in protocol["support_split_seeds"])
    if not model_seeds or not split_seeds:
        raise A251bError("A25.1a protocol has an empty seed dimension")

    derived_workers = len(DOMAINS) * len(model_seeds) * len(split_seeds)
    declared_workers = int(protocol["expected_A25_1b_worker_cells"])
    if declared_workers != derived_workers:
        raise A251bError(
            "A25.1a worker factorial mismatch: "
            f"declared={declared_workers}, derived={derived_workers}"
        )

    source_phase_cells_per_worker = (len(DOMAINS) - 1) * 2
    denominator = derived_workers * source_phase_cells_per_worker
    total_records = int(protocol["expected_source_episode_records"])
    if total_records < 1 or total_records % denominator != 0:
        raise A251bError(
            "A25.1a source-episode total cannot be evenly assigned across the "
            f"locked factorial: total={total_records}, denominator={denominator}"
        )
    per_phase = total_records // denominator
    if per_phase < 1:
        raise A251bError("derived episodes per source-domain/phase is not positive")
    return per_phase
...
def worker_tasks(tasks: pd.DataFrame, domain: str, seed: int, split: int, protocol: Mapping[str, Any]) -> pd.DataFrame:
    ...
    per_phase = episodes_per_source_domain_phase(protocol)
...
def next_full_batch(iterator, loader, expected_batch_size: int):
    """Return a full batch, cycling deterministically past any final short batch."""
    attempts = 0
    maximum_attempts = max(2, int(len(loader)) * 2 + 1)
    while attempts < maximum_attempts:
        try:
            ...
            try:
                ...
                raise A251bError("source loader produced zero batches") from exc
        attempts += 1
        if int(batch[1].numel()) == int(expected_batch_size):
            return batch, iterator
    raise A251bError(
        f"loader cannot provide a full source batch of size={expected_batch_size}"
    )
...
def ordinary_source_train(model: torch.nn.Module, schedule: Sequence[Mapping[str, Any]], normalized: Mapping[str, pd.DataFrame], cfg: Mapping[str, Any], seed: int, device: torch.device, accounting: dict[str, Any]) -> tuple[torch.nn.Module, list[dict[str, Any]]]:
    ...
    for outer_index, row in enumerate(schedule, start=1):
        ...
        for _ in range(int(cfg["inner_steps"])):
            batch, iterator = next_full_batch(iterator, loader, int(cfg["batch_size"]))
...
def worker_outputs_are_complete(
    directory: Path,
    expected_contract_hashes: Mapping[str, str],
    expected_records: int,
) -> bool:
    status_path = directory / "worker_status.json"
    run_path = directory / "run_level.csv"
    try:
        if not worker_status_is_compatible(status_path, expected_contract_hashes):
            return False
        status = read_json(status_path, label="worker status")
        if int(status.get("completed_run_records", -1)) != int(expected_records):
            return False
        run = load_csv(run_path, label="worker run level")
        if len(run) != int(expected_records):
            return False
        if {"checkpoint", "checkpoint_sha256"} - set(run.columns):
            return False
        if run["checkpoint"].duplicated().any():
            return False
        root = directory.resolve()
        for row in run[["checkpoint", "checkpoint_sha256"]].to_dict(orient="records"):
            ...
            if HASH_RE.fullmatch(expected_hash) is None or sha256(checkpoint) != expected_hash:
                return False
        for name in ("training_history.csv", "runtime_parameter_audit.csv", "source_normalizer.json"):
            path = directory / name
            if not path.is_file() or path.stat().st_size == 0:
                return False
    except (A251bError, OSError, ValueError, TypeError):
        return False
    return True
...
def run_worker(args: argparse.Namespace) -> None:
    ...
    status_path = directory / "worker_status.json"
    run_path = directory / "run_level.csv"
    expected_worker_records = len(METHODS) * len(protocol["shots"])
    if args.resume and worker_outputs_are_complete(
        directory, contract_hashes, expected_worker_records
    ):
    ...
    for architecture, (ordinary, reptile) in PAIRS.items():
        ...
        if method_audit[ordinary]["initial_state_sha256"] != method_audit[reptile]["initial_state_sha256"]:
    ...
    atomic_csv(run_path, run_rows)
    atomic_dataframe(directory / "training_history.csv", pd.DataFrame(histories))
    atomic_csv(directory / "runtime_parameter_audit.csv", audit_rows)
...
def verify_worker_preflight(
    ...
    roles = contract_frames["roles"]
    declared_workers = int(protocol["expected_A25_1b_worker_cells"])
    if len(workers) != declared_workers or len(set(workers)) != declared_workers:
        raise A251bError(
            f"worker factorial mismatch: derived={len(workers)}, declared={declared_workers}"
        )
    expected_records = len(workers) * len(METHODS) * len(protocol["shots"])
    if expected_records != int(protocol["expected_A25_1b_training_records"]):
        raise A251bError(
            "training-record factorial mismatch: "
            f"derived={expected_records}, declared={protocol['expected_A25_1b_training_records']}"
        )
    expected_source_updates = int(protocol["outer_steps"]) * int(protocol["inner_steps"])
    if expected_source_updates != int(protocol["source_gradient_updates_per_method_cell"]):
        raise A251bError("source gradient budget is inconsistent with outer_steps * inner_steps")
    expected_source_windows = expected_source_updates * int(cfg["batch_size"])
    if expected_source_windows != int(protocol["source_window_presentations_per_method_cell"]):
        raise A251bError("source window budget is inconsistent with updates * batch_size")
    ...
    for domain, seed, split in workers:
        ...
        confirmation = role_engines(roles, domain, split, "confirmation")
        support_union = set().union(*(set(values) for values in support.values()))
        if set(selection) & support_union:
            raise A251bError("worker target role leakage")
        if set(confirmation) & (set(selection) | support_union):
            raise A251bError("worker confirmation role is not sealed from support/selection")
        target_frame = frames[domain]
        for shot, engines in support.items():
            dataset = a23.WindowDataset(target_frame, engines, int(cfg["window_size"]))
            if len(dataset) < 1:
                raise A251bError(f"empty target support dataset: {domain}/{split}/K={shot}")
        selection_dataset = a23.WindowDataset(target_frame, selection, int(cfg["window_size"]))
        if len(selection_dataset) < 1:
            raise A251bError(f"empty target selection dataset: {domain}/{split}")

        # Source training explicitly skips a final short batch.  Every episode
        # must therefore contain at least one full batch before workers launch.
        for row in subset.loc[subset["episode_phase"].astype(str) == "meta_train"].to_dict(orient="records"):
            source = str(row["source_domain"])
            episode_support = parse_engine_json(
                row["meta_support_engine_ids"], label="meta_support_engine_ids"
            )
            dataset = a23.WindowDataset(frames[source], episode_support, int(cfg["window_size"]))
            required_windows = int(cfg["batch_size"])
            if len(dataset) < required_windows:
                raise A251bError(
                    "source episode cannot supply the locked full-batch schedule: "
                    f"target={domain}, source={source}, split={split}, "
                    f"available={len(dataset)}, required={required_windows}"
                )

        # Exercise the exact child CLI construction and parser used by GPU jobs.
        child_argv = worker_command(args, domain, seed, split)[2:]
        child_args = parse_args(child_argv)
        if not child_args.worker or child_args.target_domain != domain:
            raise A251bError("worker command/parser roundtrip failed")
    return workers
...
def merge(args: argparse.Namespace, protocol: Mapping[str, Any], workers: Sequence[tuple[str, int, int]], contract_hashes: Mapping[str, str]) -> None:
    ...
    for domain, seed, split in workers:
        ...
        if not worker_outputs_are_complete(directory, contract_hashes, expected_per_worker):
            raise A251bError(f"incomplete, incompatible, or corrupt worker: {directory.name}")
    ...
    atomic_dataframe(run_path, merged)
    atomic_dataframe(audit_path, pd.concat(audits, ignore_index=True).drop_duplicates())
    atomic_dataframe(history_path, pd.concat(histories, ignore_index=True))
    atomic_dataframe(matched_path, pd.DataFrame(pair_rows))
    decision = {
        "experiment_id": EXPERIMENT_ID,
        "complete": True,
        "passed": True,
...
def parent(args: argparse.Namespace) -> None:
    ...
    raw = load_frames(args, protocol, cfg)
    workers = verify_worker_preflight(args, protocol, raw, frames_contract, cfg)
