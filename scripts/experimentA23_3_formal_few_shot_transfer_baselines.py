...
SCRIPT_VERSION = "experimentA23_3_formal_few_shot_transfer_baselines_v2"
...
def load_protocol(args: argparse.Namespace) -> tuple[dict[str, Any], pd.DataFrame, dict[str, str], dict[str, dict[str, str]]]:
    ...
    # A23.0 assigns support_rank only to support_pool engines.  Empty ranks on
    # selection/confirmation rows are part of the locked protocol, not corrupt
    # data, so retain the column as pandas' nullable integer dtype.
    ranks = pd.to_numeric(roles["support_rank"], errors="coerce")
    support_mask = roles["role"].eq("support_pool")
    if ranks.loc[support_mask].isna().any():
        bad = roles.loc[support_mask & ranks.isna(), [
            "target_domain", "support_split_seed", "engine_id", "role", "support_rank"
        ]]
        raise A233Error(
            "A23.0 contains a support_pool engine without an integer support_rank: "
            f"{bad.head(10).to_dict('records')}"
        )
    non_null_ranks = ranks.dropna().to_numpy(dtype=np.float64)
    if not np.all(np.equal(non_null_ranks, np.floor(non_null_ranks))):
        raise A233Error("A23.0 support_rank contains a non-integer value")
    roles["support_rank"] = ranks.astype("Int64")
