...
retrospectively sampled, causal input prefixes (25%, 50%, and 75% of the
observed training trajectory).  The prefix location is used only to define an
offline benchmark endpoint; the model input contains no observation after the
selected cycle.
...
def prefix_label(fraction: float) -> str:
    return f"prefix_{int(round(fraction * 100)):03d}pct"
...
class CausalPrefixDataset(Dataset):
    ...
    def __init__(
        ...
        for unit in sorted(wanted):
            ...
            for fraction in REGISTERED_PREFIXES:
                endpoint = max(0, min(n_cycles - 1, int(math.ceil(fraction * n_cycles)) - 1))
                endpoint_indices.append(endpoint)
            if len(set(endpoint_indices)) != len(REGISTERED_PREFIXES):
                raise A232Error(
                    f"registered prefixes collapse to duplicate cycles for unit={unit}: {endpoint_indices}"
                )
            features = group.loc[:, a23.FEATURE_COLUMNS].to_numpy(dtype=np.float32, copy=True)
            for fraction, endpoint in zip(REGISTERED_PREFIXES, endpoint_indices):
                ...
                self.meta.append(
                    {
                        ...
                        "prefix_label": prefix_label(fraction),
                        "true_rul": truth,
        ...
        if len(self.x) != len(wanted) * len(REGISTERED_PREFIXES):
...
def preflight(
    ...
    result = {
        ...
        "registered_primary_question": (
            ...
            "pre-registered causal trajectory prefixes that cover high, mid, and low RUL?"
        ),
        ...
        "registered_prefix_fractions": list(REGISTERED_PREFIXES),
        "rul_stages": {
            "high_rul_gt60": "> 60",
        ...
        "mixed_prefix_fractions_within_inference_batch": False,
        "prefix_sampling_uses_complete_trajectory_only_for_offline_endpoint_definition": True,
...
def infer_checkpoint(
    ...
    # put an engine's 25%, 50%, and 75% endpoints in the same graph/batch:
    # doing so would let an earlier prefix depend on a later prefix.  Each
    # registered fraction is inferred separately and in stable engine order.
    for fraction in REGISTERED_PREFIXES:
        subset_indices = [
            index
            for index, meta in enumerate(dataset.meta)
            if math.isclose(
                float(meta["prefix_fraction"]), float(fraction), abs_tol=1e-12, rel_tol=0.0
            )
        ]
        if len(subset_indices) * len(REGISTERED_PREFIXES) != len(dataset):
            raise A232Error(
                f"prefix subset cardinality mismatch for fraction={fraction}: {path}"
            )
        ...
        for x, indices in loader:
            try:
                ...
                raise A232Error(
                    f"model forward failed for {path}, prefix={fraction}: {exc}"
                ) from exc
...
def evaluation_scopes() -> list[tuple[str, str, Any]]:
    ...
    scopes.extend((prefix_label(value), "prefix", value) for value in REGISTERED_PREFIXES)
    scopes.extend(
        (stage, "stage", stage)
...
def select_scope(frame: pd.DataFrame, kind: str, value: Any) -> pd.DataFrame:
    if kind == "all":
        return frame
    if kind == "prefix":
        return frame.loc[np.isclose(frame["prefix_fraction"].astype(float), float(value))]
    if kind == "stage":
