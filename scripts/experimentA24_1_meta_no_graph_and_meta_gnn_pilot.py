...
Source meta-query engines are validation-only.  Target selection engines are
used only for pilot implementation diagnostics; target confirmation engines
are not evaluated in A24.1 and remain sealed for A24.2.
"""
...
SCRIPT_VERSION = "experimentA24_1_meta_no_graph_and_meta_gnn_pilot_v2"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    if args.min_free_memory_mb < 0 or not (0 <= args.max_gpu_utilization <= 100):
        raise A24Error("invalid GPU memory/utilization threshold")
...
def strict_bool_series(frame: pd.DataFrame, column: str) -> pd.Series:
    """Parse a persisted Boolean column without Python's bool('False') trap."""
    if column not in frame:
        raise A24Error(f"required Boolean column is missing: {column}")
    values = frame[column]
    ...
    if unexpected:
        raise A24Error(f"invalid Boolean values in {column}: {unexpected[:10]}")
...
def load_contract(args: argparse.Namespace):
    ...
    for col in required_true:
        if not strict_bool_series(tasks, col).all():
    ...
    for col in ("engine_selection_uses_labels", "engine_selection_uses_trajectory_length",
        ...
        if strict_bool_series(tasks, col).any():
            raise A24Error(f"A24.0 task contract violation: {col}")
    if not strict_bool_series(audits, "all_target_role_sets_disjoint").all():
        raise A24Error("target engine roles are not disjoint")
    if strict_bool_series(audits, "target_engines_allowed_in_meta_training").any():
...
def a23_namespace(args: argparse.Namespace) -> argparse.Namespace:
    """Return the complete Namespace contract required by reused A23 helpers."""
    local = deepcopy(args)
    ...
    local.shots = (int(args.shot),)
    local.support_split_seeds = tuple(int(x) for x in args.support_split_seeds)
    return local


def configure(args: argparse.Namespace, protocol: dict[str, Any]) -> dict[str, Any]:
    local = a23_namespace(args)
    cfg = a23.load_config(local)
    if args.outer_steps is not None and int(args.outer_steps) != int(protocol["outer_steps"]):
        raise A24Error("--outer-steps cannot change the locked A24.0 value")
    if args.inner_steps is not None and int(args.inner_steps) != int(protocol["inner_steps"]):
        raise A24Error("--inner-steps cannot change the locked A24.0 value")
    if (args.outer_learning_rate is not None
            and not math.isclose(float(args.outer_learning_rate),
                                 float(protocol["outer_learning_rate"]),
                                 rel_tol=0.0, abs_tol=1e-15)):
        raise A24Error("--outer-learning-rate cannot change the locked A24.0 value")
...
def parameter_audit(cfg: dict[str, Any], seed: int) -> list[dict[str, Any]]:
    ...
    value_mismatch = [k for k in shared
                      if not torch.equal(named[METHODS[0]][k].detach().cpu(),
                                         named[METHODS[1]][k].detach().cpu())]
    if value_mismatch:
        raise A24Error(f"shared parameter initialisation mismatch: {value_mismatch[:10]}")
    ...
    for method, model in models.items():
        ...
        rows.append({"method": method, "total_parameters": total,
                     ...
                     "shared_parameter_initialization_identical": True})
...
def load_raw_frames(args: argparse.Namespace, protocol: dict[str, Any], cfg: dict[str, Any]):
    """Load only C-MAPSS training files and verify A24.0's immutable hashes."""
    data_dir = resolve(args.data_dir)
    locked = protocol.get("training_file_inventory")
    if not isinstance(locked, dict) or set(locked) != set(DOMAINS):
        raise A24Error("A24.0 training_file_inventory is incomplete")
    frames: dict[str, pd.DataFrame] = {}
    for domain in DOMAINS:
        path = a23.resolve_train_file(data_dir, domain)
        expected_hash = str(locked[domain].get("sha256", ""))
        actual_hash = sha256(path)
        if actual_hash != expected_hash:
            raise A24Error(
                f"training file hash mismatch for {domain}: expected={expected_hash} actual={actual_hash}"
            )
        frame = a23.load_domain_frame(path, rul_cap=cfg["rul_cap"])
        actual_engines = int(frame["unit"].nunique())
        expected_engines = int(locked[domain].get("n_engines", -1))
        if actual_engines != expected_engines:
            raise A24Error(
                f"training engine count mismatch for {domain}: expected={expected_engines} actual={actual_engines}"
            )
        frames[domain] = frame
    return frames
...
def validate_worker_tasks(
    ...
    *,
    ...
    expected_per_phase = int(protocol["episodes_per_source_domain_per_phase"])
    expected_sources = set(DOMAINS) - {target}
    if set(subset["source_domain"].astype(str)) != expected_sources:
        raise A24Error(f"source-domain set mismatch for target={target}")
    counts = subset.groupby(["source_domain", "episode_phase"]).size().to_dict()
    for source in sorted(expected_sources):
        for phase in ("meta_train", "meta_validation"):
            if int(counts.get((source, phase), 0)) != expected_per_phase:
                raise A24Error(f"task count mismatch target={target} source={source} phase={phase}")
        phase_unions: dict[str, set[int]] = {}
        valid_engines = set(map(int, raw_frames[source]["unit"].unique()))
        for phase in ("meta_train", "meta_validation"):
            phase_rows = subset.loc[
                (subset["source_domain"] == source) & (subset["episode_phase"] == phase)
            ]
            union: set[int] = set()
            for row in phase_rows.itertuples(index=False):
                ...
                if len(support) != int(protocol["meta_support_engines_per_episode"]):
                    raise A24Error("meta-support engine count differs from A24.0")
                if len(query) != int(protocol["meta_query_engines_per_episode"]):
                    raise A24Error("meta-query engine count differs from A24.0")
                if set(support) & set(query):
                    raise A24Error("episode support/query leakage")
                if not (set(support) | set(query)) <= valid_engines:
                    raise A24Error(f"episode refers to unknown {source} engine")
                union.update(support); union.update(query)
            phase_unions[phase] = union
        if phase_unions["meta_train"] & phase_unions["meta_validation"]:
            raise A24Error(f"meta-train/meta-validation engine leakage in {source}")


def model_runtime_smoke(cfg: dict[str, Any], seed: int) -> list[dict[str, Any]]:
    """Run CPU forward/backward and checkpoint round-trip for both methods."""
    rows: list[dict[str, Any]] = []
    batch = max(8, int(cfg.get("graph_k", 5)) + 1)
    x = torch.randn(batch, int(cfg["window_size"]), len(a23.FEATURE_COLUMNS))
    y = torch.linspace(10.0, 100.0, batch)
    with tempfile.TemporaryDirectory(prefix="a24_1_smoke_") as temporary:
        for method in METHODS:
            model = make_model(method, cfg, seed).cpu()
            model.train()
            loss, prediction = rul_training_loss(model, x, y, cfg["pair_aux_weight"])
            if tuple(prediction.reshape(-1).shape) != (batch,) or not torch.isfinite(loss):
                raise A24Error(f"non-finite or wrong-shaped smoke output for {method}")
            loss.backward()
            ...
            if not gradients or any(not torch.isfinite(g).all() for g in gradients.values()):
                raise A24Error(f"missing/non-finite gradient in smoke test for {method}")
            required_prefixes = ["temporal.", "predictor."]
            if method == "meta_gnn_k":
                required_prefixes.append("gat.")
            for prefix in required_prefixes:
                if not any(name.startswith(prefix) for name in gradients):
                    raise A24Error(f"smoke test produced no {prefix} gradient for {method}")
            path = Path(temporary) / f"{method}.pt"
            torch.save({"state": {k: v.detach().cpu() for k, v in model.state_dict().items()}}, path)
            payload = torch.load(path, map_location="cpu", weights_only=False)
            verifier = make_model(method, cfg, seed).cpu()
            verifier.load_state_dict(payload["state"], strict=True)
            rows.append({"method": method, "forward_passed": True,
                         "backward_passed": True, "checkpoint_roundtrip_passed": True,
...
def acquire_parent_lock(root: Path) -> Path:
    root.mkdir(parents=True, exist_ok=True)
    path = root / "experimentA24_1_run.lock"
    if path.exists():
        try:
            ...
            raise A24Error(f"cannot verify existing A24.1 lock: {path}") from exc
        else:
            raise A24Error(f"another A24.1 parent is running with pid={old_pid}")
...
def release_parent_lock(path: Path) -> None:
    if not path.exists():
        return
    try:
...
def run_worker(args: argparse.Namespace) -> None:
    ...
    if args.resume and status_path.is_file() and run_path.is_file():
        ...
        if (status.get("complete") and status.get("passed")
            ...
            return
        print(f"[A24.1] resume found incompatible/incomplete shard; rerunning {out.name}", flush=True)
    ...
    raw = load_raw_frames(args, protocol, cfg)
    validate_worker_tasks(subset, target=target, protocol=protocol, raw_frames=raw)
    ...
    # A24.1 is an implementation pilot.  Evaluate only the target selection
    # engines; keep confirmation engines sealed for the formal A24.2 analysis.
    selection_loader = a23.make_loader(
        a23.WindowDataset(frames[target], selection, cfg["window_size"]),
    ...
    for method in METHODS:
        ...
        metrics = a23.evaluate(model, selection_loader, device)
    ...
    a23.atomic_json(status_path, {"experiment_id": EXPERIMENT_ID,
                                  "script_version": SCRIPT_VERSION,
                                  "complete": True,
...
def merge(args, workers):
    ...
    if len(merged) != expected or strict_bool_series(merged, "checkpoint_reload_passed").sum() != expected:
        raise A24Error("merged pilot record/checkpoint count mismatch")
    for column in ("confirmation_used_for_training", "confirmation_used_for_evaluation",
                   "selection_used_for_training", "official_test_files_accessed",
                   "official_test_forward_run"):
        if strict_bool_series(merged, column).any():
            raise A24Error(f"merged integrity violation: {column}")
    if not strict_bool_series(merged, "selection_used_for_evaluation").all():
        raise A24Error("selection-only pilot evaluation contract was not followed")
...
def parent(args):
    protocol, tasks, hashes = load_contract(args)
    cfg = configure(args, protocol)
    # Exercise the complete downstream A23 interfaces before scheduling any
    # worker so Namespace/data/model failures cannot survive preflight.
    compatibility = a23_namespace(args)
    _, roles, _ = a23.load_protocol(compatibility)
    ...
    for domain, seed, split in workers:
        subset = tasks.loc[(tasks["target_domain"] == domain)
                           & (tasks["model_seed"].astype(int) == seed)
                           & (tasks["target_support_split_seed"].astype(int) == split)].copy()
        expected = ((len(DOMAINS) - 1) * 2
                    * int(protocol["episodes_per_source_domain_per_phase"]))
        if len(subset) != expected:
            raise A24Error(
                f"preflight worker task count mismatch domain={domain} seed={seed} split={split}"
            )
        validate_worker_tasks(subset, target=domain, protocol=protocol, raw_frames=raw)
        support = a23.role_engines(roles, domain, split, "support_pool", args.shot)
        selection = a23.role_engines(roles, domain, split, "selection")
        confirmation = a23.role_engines(roles, domain, split, "confirmation")
        if len(support) != args.shot or set(support) & (set(selection) | set(confirmation)):
            raise A24Error(f"preflight target-role mismatch domain={domain} split={split}")
    smoke = model_runtime_smoke(cfg, args.model_seeds[0])
    preview = {"experiment_id": EXPERIMENT_ID, "script_version": SCRIPT_VERSION,
               ...
               "runtime_parameter_audit": audit,
               "runtime_model_smoke": smoke,
               "all_worker_task_subsets_preflighted": True,
               "training_file_hashes_and_engine_counts_verified": True,
               "selection_only_diagnostics": True,
               "confirmation_engines_evaluated": False,
    ...
    if args.device == "cpu":
        ...
        while pending or active:
            for gpu in eligible:
                ...
                # Each scheduled attempt receives a clean log. Completed,
                # compatible shards are skipped inside the worker before any
                # scientific artifact is overwritten.
                handle = (d / "worker_training.log").open("w", encoding="utf-8")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.worker:
        ...
        lock = acquire_parent_lock(resolve(args.output_dir))
        try:
            parent(args)
        finally:
            release_parent_lock(lock)
    return 0
